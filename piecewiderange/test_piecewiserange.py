...
class PiecewiseRangeTests(unittest.TestCase):
    ...
    def test_three_ranges(self):
        self.assertEqual(
            list(PiecewiseRange("1-3,4-6,8-10")), [1, 2, 3, 4, 5, 6, 8, 9, 10],
        )

    def test_single_digit_range_and_ranges(self):
        self.assertEqual(
            list(PiecewiseRange("1-2,4,8-10")), [1, 2, 4, 8, 9, 10],
        )
    ...
    def test_equality(self):
        ...
        self.assertEqual(
            PiecewiseRange("1-2, 6-8, 12-14"), PiecewiseRange("1-2, 6-8, 12-14"),
        )
        self.assertEqual(
        ...
        self.assertEqual(
            repr(PiecewiseRange("1,2,3")), repr(PiecewiseRange("1-3")),
        )
        self.assertEqual(
...
class PiecewiseRangeShrinkRangeTests(unittest.TestCase):
    def test_trim_ungrouped(self):
        self.assertEqual(PiecewiseRange.shrink_ranges("1"), "1")
        self.assertEqual(PiecewiseRange.shrink_ranges("1,3,5,7"), "1,3,5,7")

    @unittest.skip
    def test_trim_groupped(self):
        self.assertEqual(PiecewiseRange.shrink_ranges([1, 2, 3]), "1-3")

    @unittest.skip
    def test_trim_grouped_and_ungrouped(self):
        self.assertEqual(
            PiecewiseRange.shrink_ranges([1, 2, 3, 5, 7, 9, 10, 11]), "1-3,5,7,9-11"
        )
